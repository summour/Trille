...
// ---- Activity (cl) reorder ----
// Strategy: move DOM nodes visually during dragover (no re-render),
// then commit final order to clItems on dragend.
let clDragEl=null;
function bindClReorder(){
  ...
  editor.querySelectorAll('.clrow').forEach(row=>{
    ...
    row.addEventListener('dragend',()=>{
      ...
      clDragEl=null;
      // Commit DOM order → clItems
      const rows=[...editor.querySelectorAll('.clrow')];
      const oldItems=[...clItems];
      clItems=rows.map(r=>{
        const idx=parseInt(r.dataset.cli);
        return oldItems[idx];
      }).filter(Boolean);
      // re-assign indices so future drags work
      renderDyn();
    });
    row.addEventListener('dragover',e=>{
      if(!clDragEl||clDragEl===row)return;
      e.preventDefault();
      const rect=row.getBoundingClientRect();
      const after=e.clientY>rect.top+rect.height/2;
      if(after) row.after(clDragEl);
      else row.before(clDragEl);
    });
  });

  // Touch reorder for activities
  editor.querySelectorAll('.clrow').forEach(row=>{
    const handle=row.querySelector('.drag-handle');
    if(!handle)return;
    let timer=null,dragging=false,startY=0,dragClone=null;
    ...
    handle.addEventListener('touchmove',e=>{
      ...
      const target=els.find(el=>el.classList.contains('clrow')&&el!==row);
      if(target){
    ...
    const finish=()=>{
      ...
      const rows=[...editor.querySelectorAll('.clrow')];
      const oldItems=[...clItems];
      clItems=rows.map(r=>oldItems[parseInt(r.dataset.cli)]).filter(Boolean);
      renderDyn();
    };
...
// ---- Field (nf) reorder ----
// Same strategy: move DOM nodes, commit on dragend.
let nfDragEl=null;
function bindNfReorder(){
  ...
  editor.querySelectorAll('.nf-row').forEach((row,i)=>{
    row.dataset.nfi=String(i);

    // Desktop drag — only from handle
    row.querySelector('.nf-drag-handle')?.addEventListener('mousedown',()=>{
      row.setAttribute('draggable','true');
    });
    row.addEventListener('dragstart',e=>{
      if(!e.target.closest('.nf-drag-handle')){e.preventDefault();return;}
      nfDragEl=row;
      row.classList.add('Trille-field-reorder-dragging');
      e.dataTransfer.effectAllowed='move';
      e.dataTransfer.setData('text/plain','');
    ...
    row.addEventListener('dragend',()=>{
      ...
      row.setAttribute('draggable','false');
      nfDragEl=null;
      // Commit DOM order → nfFields
      const rows=[...editor.querySelectorAll('.nf-row')];
      const oldFields=[...nfFields];
      nfFields=rows.map(r=>oldFields[parseInt(r.dataset.nfi)]).filter(Boolean);
      renderDyn();
    });
    row.addEventListener('dragover',e=>{
      if(!nfDragEl||nfDragEl===row)return;
      e.preventDefault();
      const rect=row.getBoundingClientRect();
      const after=e.clientY>rect.top+rect.height/2;
      if(after) row.after(nfDragEl);
      else row.before(nfDragEl);
    });
  });

  // Touch reorder for fields
  editor.querySelectorAll('.nf-row').forEach(row=>{
    ...
    handle.addEventListener('touchmove',e=>{
      ...
      const touch=e.touches[0];
      const els=document.elementsFromPoint(touch.clientX,touch.clientY);
      const target=els.find(el=>el.classList.contains('nf-row')&&el!==row);
      if(target){
        const rect=target.getBoundingClientRect();
        const after=touch.clientY>rect.top+rect.height/2;
        if(after) target.after(row);
